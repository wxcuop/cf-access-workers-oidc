...
<head>
    ...
    <title>Group Management | OIDC Authentication System</title>
    <style>
        ...
        <div class="modal-content">
            ...
            <div class="modal-body">
                <div style="margin-bottom: 1rem;">
                    <strong>Group:</strong> <span id="groupUsersModalName">group-name</span>
                </div>
            ...
            <div class="modal-footer">
                ...
                <button type="button" class="btn btn-primary" onclick="saveGroupUsers()">Save Changes</button>
            </div>
        </div>
    </div>
    ...
    <script>
        ...
        function renderGroupTable(groups) {
            ...
            if (!groups || groups.length === 0) {
                tbody.innerHTML = '<tr><td colspan="5" style="text-align: center; padding: 2rem;">No groups found</td></tr>';
                return;
            }

            tbody.innerHTML = groups.map(group => {
                const memberCount = getUsersInGroup(group.name).length;
                return \`
                    <tr>
                        <td>\${group.name}</td>
                        ...
                        <td>
                            <div class="users-container">
                                <div class="user-count">
                                    <span class="count-badge">\${memberCount} members</span>
                                </div>
                                <button class="btn btn-sm btn-secondary manage-users-btn" onclick="openGroupUsersModal('\${group.name}')">
                                    Manage Users
                                </button>
                            </div>
                        </td>
                        ...
                        <td>
                            <button class="btn btn-sm btn-danger" onclick="deleteGroup('\${group.name}')">Delete</button>
                        </td>
                    </tr>
                \`;
            }).join('');
        ...
        function getUsersInGroup(groupName) {
            return currentUsers.filter(user => user.groups && user.groups.includes(groupName));
        }

        function openGroupUsersModal(groupName) {
            selectedGroup = currentGroups.find(g => g.name === groupName);
            if (!selectedGroup) return;

            document.getElementById('groupUsersModalName').textContent = groupName;
            
            const groupUsers = getUsersInGroup(groupName);
            ...
            container.innerHTML = currentUsers.map(user => \`
                <div class="user-checkbox">
                    <input type="checkbox" id="user-\${user.email}" value="\${user.email}" 
                           \${groupUsers.some(gu => gu.email === user.email) ? 'checked' : ''}>
                    <label for="user-\${user.email}">\${user.name || user.email}</label>
                </div>
            \`).join('');

            document.getElementById('groupUsersModal').style.display = 'block';
        }
        ...
        async function saveGroupUsers() {
            ...
            const checkboxes = document.querySelectorAll('#groupUsersCheckboxes input[type="checkbox"]');
            const selectedUserEmails = Array.from(checkboxes)
                .filter(cb => cb.checked)
                .map(cb => cb.value);

            try {
                // Update group users via API
                const response = await fetch(\`/admin/groups/\${selectedGroup.name}/users\`, {
                    method: 'POST',
                    headers: {
                        'Content-Type': 'application/json'
                    },
                    body: JSON.stringify({ users: selectedUserEmails })
                });

                if (response.ok) {
                    // Update local data
                    currentUsers.forEach(user => {
                        if (selectedUserEmails.includes(user.email)) {
                            if (!user.groups.includes(selectedGroup.name)) {
                                user.groups.push(selectedGroup.name);
                            }
                        } else {
                            user.groups = user.groups.filter(g => g !== selectedGroup.name);
                        }
                    });
                    
                    renderGroupTable(currentGroups);
                    closeGroupUsersModal();
                    showSuccess('Group users updated successfully');
                } else {
                    throw new Error('Failed to update group users');
                }
            } catch (error) {
                console.error('Error updating group users:', error);
                showError('Failed to update group users');
            }
        }
